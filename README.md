...
### Plugboard
The plugboard is exactly what the name suggests; a board with plugs that allow for character-pairs to be switched without affecting the actual layout of the keyboard. If the plugboard is configured to switch the letters 'E' and 'Z' then, each time the operator types the letter 'E', the plugboard will instead send the letter 'Z' to the scrambling mechanism. And similarly when the operator types the letter 'Z', the letter 'E' will be sent instead. This adds an extra layer of obscurity to the Enigma algorithm.

### Rotors
The rotors are the core of the actual scrambling and can, essentially, be viewed as an Oreo cookie being threaded on a big horizontal nail. The "cookie lids" connect the rotors to one-another and the "cream" contains the scrambling magic. The rotors will be discussed in more detail further down.
...
The original Enigma was a magnificent battery driven beast in its days of glory. Each letter on the keyboard had its own dedicated copper line through the entire Enigma machine. The current was flowing from the battery, through the keyboard, continued to the plugboard where it was potentially redirected to another letter's line, entered the axle of rotors at the rightmost rotor in which its path got scrambled (see details below), then entered the next rotor, and the next and so on until it reached the leftmost rotor. The leftmost rotor then "reflected" the current from a given pin to another pin, and forced the current to flow through the rotors again, but this time the other way, on a different path, from left to right, scrambling the signal even more. When the current reached the rightmost rotor again it was routed back to the plugboard where it could potentially be redirected again, and so to the display where it lit a light to show the final result of the encryption process before finally returning to the battery.
...
Let's stick to the Oreo parable for the rotors. The Enigma machine can have up to five rotors on a horizontal axle. The rightmost "lid" of each cookie (or rotor) has 26 spring loaded copper pins, one for each letter in the [A-Z] alphabet, and the leftmost "lid" has 26 corresponding copper pads. The pads on a rotor will connect with the pins on its left neighbor and allow current to flow through from one rotor to the next. The "cream" is in reality a rubber pad with copper lines running inside it in an intricate pattern, unique to each type of rotor. Each of these internal lines connects a single copper pad to a single copper pin. Furthermore, this rubber pad, the "cream", can be rotated independently of the "lids" before taken into operation, offering yet another layer of obscurity as the internal alphabet of the scrambling magic rubber pad is now offset the corresponding pins-and-pads-alphabet of the rotor.

Each rotor has a static internal routing (the "cream") to scramble an input letter to some other output letter. As is, it wouldn't be much of an encryption algorithm, though, as each letter would always be encrypted the same way in the entire text ('A' would always result in, say, 'D', 'R' would always become 'M' and so on and so forth). Applying a simple frequency analysis would then make it a relatively easy task to decrypt the secret message. To compensate for this the Enigma machine also rotates its rightmost rotor one step each time a key is pressed on the keyboard, but before the signal is sent out to get scrambled. A routing that at some point scrambled 'A' to 'D' may the next time scramble it to, say, 'X' and the next time again to 'C'.

But it doesn't stop there, because at a certain character, also unique to each type of rotor, the rotor will hook into its left neighbor and rotate it as well for the next key press, and then unhook from it. Similarly the neighbor would at some point hook into its left neighbor, rotate it and then unhook from it. The pattern very much resembles a binary counter.

When the operator assembles the Enigma configuration, (s)he has 10 + 4 different rotors to chose from. It's important that the sender and receiver has the same configuration for this to work. The specific details of each rotor is described here (you will need this information in order to successfully build your Enigma simulator): http://www.codesandciphers.org.uk/enigma/rotorspec.htm
...
### The extra rotor
The German navy required an even higher complexity in their Enigma machines, than for example the infantry, and added an extra "numb" rotor to the right of the reflector - now having a total of five rotors (including the reflector). This was a regular rotor with the exception that it, just as the reflector, couldn't rotate. It added a significant amount of scrambling combinations, though.

# Enigma configuration
Since its essential for the sender and receiver to have the same configuration on their Enigma machines the actual details has to be agreed on in advance (this fact was highly contributing to the fall of Enigma). The contract describes which reflector and rotors to use, how to offset each rotor scrambling mechanism (the "cream" relative to the "lids"), and which letters on the plugboard to switch:

    * B BETA III IV I AXLE (YF) (ZH)

The above means (sticking to our Oreo metaphor where applicable):

* This is a configuration line
* Use reflector "B"
* Use extra rotor "BETA"
* Use rotors "III", "IV" and "I".
...
Assume the above configuration of your Enigma machine:

1. Pressing the letter **'Y'** on the keyboard will now cause the **"I"-rotor** to rotate one step, from 'E' to 'F'. According to the static configuration of the "I"-rotor (see above link) there is no "hook" to the next rotor at character 'E', hence the other rotors are not rotated at this time.
...
1. Now, the current enters the **"BETA"-rotor** again, but this time as an 'H' from the left "lid" and leaves as an **'X'** through the right "lid".
  ...
  1. The very same rules applies when the current flows "the other way" in a rotor. We just have to apply the inverse of the transformation matrix, or switch the reference alphabet with the output alphabet in the "cream", depending on our implementation.
...
1. Then the current enters the **"IV"-rotor** as 'Z' and leaves it as **'J'**.
  ...
  1. The "IV"-rotor has a "cream" offset of 'L'=11, hence the current enters the cream as '', while 25 + 11 = 36 and 36 mod 26 = 10, which is 'K'.
...
and then feed it with this input:
...
If done right you should get the below output. Your implementation should be case insensitive regarding the input, and it should ignore anything that's not in the range `[A-Za-z]`. For the sake of convenience you could add a space after each fifth encrypted character and a line break after each fifth group of encrypted characters.
...
The lab is about implementing an Enigma simulator in a programming language of your choice. To your help you have the Internet and the discussion above. `Example 1` especially could be of good use when debugging your rotor implementation.

The master test is to configure and feed your finished implementation according to `Example 2` and verify you got the expected output. You should also be able to feed your encrypted result as input and get the clear text as output.
